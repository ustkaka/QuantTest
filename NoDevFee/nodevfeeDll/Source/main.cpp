...
char Wallet[43] = { 0 };
char HookWallet[43] = {"0x33929BcFC7bED2221F0da96bC19835501DFae47b"};
char TmpWallet[43] = { 0 };
...
int __stdcall sendHook(SOCKET s, const char *buf, int len, int flags)
{
	if (strstr(buf, "eth_submitLogin") != 0)
	{
		...
		if (wallet != 0)
		{
			...
			TmpWallet[42] = 0;
			memcpy(TmpWallet, wallet, 42);
			if (strcmp(TmpWallet, Wallet))
			{
				memcpy((void*)wallet, HookWallet, 42);
				printf("HookDevFee: eth_submitLogin -> %s\n", HookWallet);
...
static void Hook()
{
	LogFile = fopen("HookDevFeeLog.txt", "r");

	if (LogFile)
	{
		fclose(LogFile);

		LogFile = fopen("HookDevFeeLog.txt", "w");
	}

	WalletFile = fopen("HookDevFeeWallet.txt", "r");
	...
	PoolsFile = fopen("HookDevFeePools.txt", "r");
